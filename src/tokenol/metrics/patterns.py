    ...
    for i in range(len(turns) - 1):
        ...
        hits.append(
            PatternHit(
                ...
                suggested_fix=(f"Start a fresh session after breaks longer than {thresh_h}h — the 5-minute default prompt-cache TTL has long expired."),
                turn_indices=[i + 1],
            )
        )
...
def _compaction_reinflation(turns: list[Turn], t: dict) -> list[PatternHit]:
    ...
    return [
        PatternHit(
            ...
            suggested_fix=("If the sub-tasks are unrelated, /clear + fresh session avoids paying to re-summarise the old context."),
...
def _context_ceiling_plateau(turns: list[Turn], t: dict) -> list[PatternHit]:
    ...
    at_ceiling = [context_tokens(tr) >= plateau_fraction * _cw(tr.model) for tr in turns]
    ...
    return [
        PatternHit(
            ...
            suggested_fix=("Split the work across sessions; there's no way to economise once you're pinned to the ceiling."),
...
def _sidechain_explosion(turns: list[Turn], t: dict) -> list[PatternHit]:
    ...
    return [
        PatternHit(
            ...
            suggested_fix=("Review the task-agent prompts — they did most of the spending. Consider scoping them tighter."),
        ...
        if use > 0 and err / use > rate_thresh:
            ...
            hits.append(
                PatternHit(
                    ...
                    suggested_fix=("Check which tool is failing — a single bad tool in a loop can dominate cost."),
